...
# Define the path to the Demucs executable
demucs_path = "demucs"
...
def main(args):
    ...
    separate_command = f"{demucs_path} --two-stems=vocals {args.wave}"
    subprocess.run(separate_command, shell=True, check=True)

    separated_vocals = f"separated/htdemucs/{args.wave.split('/')[-1].split('.')[0]}/vocals.wav"
    separated_background = f"separated/htdemucs/{args.wave.split('/')[-1].split('.')[0]}/no_vocals.wav"

    # Step 2: Process vocals with your neural network
    if (args.ppg == None):
        args.ppg = "svc_tmp.ppg.npy"
        print(f"Auto run : python whisper/inference.py -w {separated_vocals} -p {args.ppg}")
        os.system(f"python whisper/inference.py -w {separated_vocals} -p {args.ppg}")
    ...
    model = SynthesizerInfer(
        ...
        hp)
    svc_model = load_svc_model(args.model, model)
    svc_model.eval()
    svc_model.to(device)
    ...
    pit = compute_f0_nn(separated_vocals, device)
    ...
    with torch.no_grad():
        ...
        audio = svc_model(ppg, pit, spk, len_min)
        audio = audio[0, 0].data.cpu().detach().numpy()

    # Step 3: Combine processed vocals with background audio
    combined_output_path = f"combined_output_{args.wave.split('/')[-1].split('.')[0]}.wav"
    # boost audio volume 2x
    audio = audio * args.volume
    write("svc_out.wav", hp.data.sampling_rate, audio)
    # combine with ffmpeg
    combine_command = f"ffmpeg -i {separated_background} -i svc_out.wav -filter_complex amix=inputs=2:duration=longest {combined_output_path}"
